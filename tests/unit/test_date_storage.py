    ...
    try:
        ...
        for idx in range(rows):
            conn.execute(
                ...
                VALUES (?, 'techblog', 'Test', ?, ?, 'summary', NULL, ?, '{}')
                """,
                [
                    idx + 1,
                    f"Title {idx + 1}",
...
def test_snapshot_database_creates_file(tmp_path: Path) -> None:
    from blogradar.date_storage import snapshot_database
...
def test_latest_snapshot_path_returns_newest_daily_duckdb(tmp_path: Path) -> None:
    from blogradar.date_storage import latest_snapshot_path

    db_file = tmp_path / "data" / "radar_data.duckdb"
    older = tmp_path / "data" / "daily" / "2026-04-09.duckdb"
    newer = tmp_path / "data" / "daily" / "2026-04-10.duckdb"
    ignored = tmp_path / "data" / "daily" / "not-a-date.duckdb"
...
def test_resolve_read_database_path_prefers_primary_with_articles(tmp_path: Path) -> None:
    from blogradar.date_storage import resolve_read_database_path

    db_file = tmp_path / "data" / "radar_data.duckdb"
    snapshot = tmp_path / "data" / "daily" / "2026-04-10.duckdb"
...
def test_resolve_read_database_path_falls_back_when_primary_has_no_articles(
    tmp_path: Path,
) -> None:
    from blogradar.date_storage import resolve_read_database_path
    ...
    db_file = tmp_path / "data" / "radar_data.duckdb"
    snapshot = tmp_path / "data" / "daily" / "2026-04-10.duckdb"
    _init_crawl_health_only_db(db_file)
    _init_articles_db(snapshot, rows=2)
...
def test_cleanup_date_directories_removes_old(tmp_path: Path) -> None:
    from blogradar.date_storage import cleanup_date_directories
...
def test_cleanup_dated_reports(tmp_path: Path) -> None:
    from blogradar.date_storage import cleanup_dated_reports
