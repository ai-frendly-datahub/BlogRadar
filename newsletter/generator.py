...
def get_matched_groups(
    ...
    # Also check entities
    for _entity_name, entity_keywords in article.get("entities", {}).items():
...
def generate_newsletter(
    ...
    korean_articles = [a for a in articles if is_korean_source(a["source"], korean_sources)][:10]

    # Global company articles
    global_articles = [a for a in articles if is_global_source(a["source"], global_sources)][:10]
...
def render_markdown(newsletter_data: dict[str, Any]) -> str:
    ...
    if newsletter_data.get("domain_groups"):
        ...
        for domain, articles in newsletter_data["domain_groups"].items():
            if articles:
                ...
                for article in articles:
                    lines.append(
                        f"- [{article['title']}]({article['link']}) " f"*({article['source']})*"
                    )
                lines.append("")
    ...
    if newsletter_data.get("korean_articles"):
        ...
        for article in newsletter_data["korean_articles"]:
            lines.append(f"- [{article['title']}]({article['link']}) " f"*({article['source']})*")
        lines.append("")
    ...
    lines.append("---")
    lines.append("")
    lines.append("*Generated by [BlogRadar](https://github.com/ai-frendly-datahub/BlogRadar)*")
...
def main() -> None:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Generate weekly newsletter from BlogRadar data")
    parser.add_argument(
        "--db",
        type=Path,
